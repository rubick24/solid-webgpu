...
import type { Mat2dLike, Vec2Like } from './types'

/**
 * A 2x3 Matrix
 */
export class Mat2d extends Float32Array {
  static IDENTITY_2X3 = new Float32Array([1, 0, 0, 1, 0, 0])
  ...
  constructor(...values: [Readonly<Mat2dLike> | ArrayBufferLike, number?] | number[]) {
    switch (values.length) {
      case 6:
        ...
        break
      case 2:
        super(values[0] as ArrayBufferLike, values[1], 6)
        break
      case 1:
        const v = values[0]
        if (typeof v === 'number') {
   ...
   * @returns `out`
   */
  static multiplyScalarAndAdd<T extends Mat2dLike>(
    out: T,
