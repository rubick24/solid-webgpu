...
const formLineVertices = (options: {
  pointA: Vec3Like
  pointB: Vec3Like
  lineWidth: number
  segments: number
  ...
  const { pointA, pointB, lineWidth, segments, camera } = options
  const vertices = []
  const indices = []
  let vertexCount = 0
  ...
  const normal = Vec3.cross(Vec3.create(), direction, camera.up()).normalize()

  vertices.push(
    // position, tangent, normal, side, isEndpoint, endpointAngle
    [pointA, direction, normal, -1.0, 0, 0.0],
    [pointA, direction, normal, 1.0, 0, 0.0],
    [pointB, direction, normal, -1.0, 0, 0.0],
    [pointB, direction, normal, 1.0, 0, 0.0]
  )
  // main box
  ...
  vertices.push([pointA, direction, normal, 0.0, 1, 0.0]) // 中心点
  vertexCount++
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    vertices.push([
      ...
      direction,
      normal,
      i === 0 ? -1.0 : i === segments ? 1.0 : Math.sin(angle),
      1,
      angle / Math.PI
    ])
  ...
  vertices.push([pointB, direction, normal, 0.0, 2, 0.0]) // 中心点
  vertexCount++
...
const App = () => {
  ...
  const lineData = new Float32Array([0, 0, 0, 0, 0, 1])
